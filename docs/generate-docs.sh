...
MAN_FILE="$SCRIPT_DIR/install-binary.1"
...
if command -v groff &> /dev/null; then
    ...
    groff -man -Thtml "$MAN_FILE" > "$OUTPUT_DIR/install-binary.html"
    echo "  ✓ HTML generated: $OUTPUT_DIR/install-binary.html"
else
...
if command -v groff &> /dev/null && command -v ps2pdf &> /dev/null; then
    echo "Generating PDF..."
    groff -man -Tps "$MAN_FILE" | ps2pdf - "$OUTPUT_DIR/install-binary.pdf"
    echo "  ✓ PDF generated: $OUTPUT_DIR/install-binary.pdf"
else
...
if command -v man &> /dev/null; then
    ...
    MANWIDTH=80 man "$MAN_FILE" | col -b > "$OUTPUT_DIR/install-binary.txt"
    echo "  ✓ Plain text generated: $OUTPUT_DIR/install-binary.txt"
else
...
if command -v pandoc &> /dev/null; then
    ...
    pandoc -f man -t markdown "$MAN_FILE" -o "$OUTPUT_DIR/install-binary.md"
    echo "  ✓ Markdown generated: $OUTPUT_DIR/install-binary.md"
else
